      ...
      cy.get("color-legend").each(($el, idx) => {
        cy.wrap($el)
          ...
          .within(() => {
            ...
            cy.get("line").then(($el) => {
              expect($el.length).equals(legendConfigs[idx].ticks);
            });

            // check if the rendered tick values are the same as the values configured
            const stringTickValues = legendConfigs[idx].tickValues.map((val) =>
